...
public class ETagFilter implements Filter {
	...
	private static final String FILTERAPPLIED = "com.vasworks.servlet.html.filter.etag.APPLIED_ONCE";
	...
	public void doFilter(ServletRequest rq, ServletResponse rs, FilterChain chain) throws IOException, ServletException {
		...
		if (authentication != null && authentication.isAuthenticated()) {
			LOG.info("AppUser " + authentication.getName() + " is/was authenticated, set response cache headers to private.");
			...
			if (request.getMethod().equalsIgnoreCase("get")) {
				if (bufferingResponse.isCookieAdded()) {
					LOG.warn("GET method, but cookie added!");
					response.addHeader("Cache-Control", "private");
					response.addHeader("Expires", "-1");
				}
			} else {
		...
		if (!noCache && eTag != null && eTag.equals(noneMatch)) {
			LOG.info("Not modified " + request.getRequestURI());
			...
			if (bufferingResponse.getHeader("ETag") == null) {
				...
				if (!bufferingResponse.isCookieAdded()) {
					...
					LOG.warn("Not adding ETag, cookie added " + bufferingResponse.isCookieAdded());
				}
			}
			response.getWriter().write(new String(byteArray));
		}
	}

 ...
 * @author developer
 */
public class KerberosProcessingFilter extends GenericFilterBean implements InitializingBean, CallbackHandler {
	...
	public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain) throws IOException,
		...
		if (((HttpServletRequest) request).getRequestURI().substring(((HttpServletRequest) request).getContextPath().length()).startsWith("/j_")) {
			LOG.info("Ignoring " + ((HttpServletRequest) request).getRequestURI().substring(((HttpServletRequest) request).getContextPath().length()));
			chain.doFilter(request, response);
			return;
		}
		...
		final String authMessage = ((HttpServletRequest) request).getHeader("Authorization");
		LOG.info("Request: " + ((HttpServletRequest) request).getRequestURI() + " AUTH: " + authMessage);
		...
		if (kerberosState == null) {
			if (forceIdentification) {
				...
				restartKerberosAuthentication((HttpServletRequest) request, (HttpServletResponse) response, session);
				return;
			} else {
		...
		if (kerberosState == COMPLETE && this.reAuthOnIEPost((HttpServletRequest) request))
			kerberosState = BEGIN;
				...
				if (tokType == SPNEGO.NegTokenInit) {
					...
					try {
						...
						if (oidStr != null && (oidStr.equals(OID.ID_MSKERBEROS5) || oidStr.equals(OID.ID_KERBEROS5))) {
							...
							KerberosAuthenticationToken token = doKerberosLogon(negToken, (HttpServletRequest) request, (HttpServletResponse) response, session);
							if (token != null) {
								...
								if (myCurrentAuth == null || myCurrentAuth instanceof AnonymousAuthenticationToken) {
									...
									try {
										this.authenticate((HttpServletRequest) request, (HttpServletResponse) response, session, token);
					...
					LOG.info("Unknown SPNEGO token type: " + tokType);
				}
			}
		}
	...
	private boolean reAuthOnIEPost(final HttpServletRequest request) {
		String ua = request.getHeader("AppUser-Agent");
		...
		try {
			...
			if (result != null) {
				...
				if (negTokenTarg != null) {
					...
					// AppUser user = createUserEnvironment(httpSess, krbDetails.getUserName());

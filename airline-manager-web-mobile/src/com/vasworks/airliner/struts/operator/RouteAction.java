...
public class RouteAction extends OperatorAction {
	...
	private void initSelAirportCodes() {
		if(entity != null) {
			...
			list.add((Airport) operatorService.find(selAirportCodes[0], Airport.class));
			list.add((Airport) operatorService.find(selAirportCodes[1], Airport.class));
			selAirportCodes = new String[list.size()];
			
			int i = 0;
			for(Airport o : list) {
				selAirportCodes[i] = o.getAirportCode();
				i++;
			}
		}
	}
	...
	public Collection<? extends RateInterface> getTaxLov() {
		return operatorService.listNonAutoTaxes(getAirline().getId());
	}
	
	public Collection<? extends RateInterface> getRebateLov() {
		return operatorService.listNonAutoRebates(getAirline().getId());
	}

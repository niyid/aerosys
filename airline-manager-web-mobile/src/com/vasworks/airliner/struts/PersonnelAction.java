...
public class PersonnelAction extends OperatorAction {
	...
	@Validations(
		requiredFields = {
			@RequiredFieldValidator(fieldName = "newUser.firstName", message = "First name is required."),
			@RequiredFieldValidator(fieldName = "newUser.lastName", message = "Last name is required."),
			@RequiredFieldValidator(fieldName = "newUser.mail", message = "Email is required."),
			@RequiredFieldValidator(fieldName = "newUser.userName", message = "AppUser-name is required."),
			@RequiredFieldValidator(fieldName = "newUser.password", message = "Password is required.")
		},
		emails = {
	...
	public String save() {
		...
		try {
			operatorService.savePersonnel(personnelId, newUser, airlineId, null, getUser().getId());
			if(personnelId == null) {

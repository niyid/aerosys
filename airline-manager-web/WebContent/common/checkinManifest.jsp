...
<div class="panel panel-primary">
	...
	<div class="panel-body">
		<s:form id="checkinForm" action="popupConfirmCheckin" namespace="/">
						...
						<table class="table table-hover table-striped">
							...
							<s:iterator value="entity.tickets" status="status">
								...
								<tr>
									...
									<s:else>
										<td><s:textfield name="entity.tickets[%{#status.index}].passenger.carryOnWeight" required="true" cssStyle="width: 70px;" /></td>
										<td><s:textfield name="entity.tickets[%{#status.index}].passenger.luggageCount" required="true" cssStyle="width: 70px;" /></td>
										<td><s:textfield name="entity.tickets[%{#status.index}].passenger.luggageWeight" required="true" cssStyle="width: 70px;" /></td>
									</s:else>
									<td>
										<s:if test="#status.index == ticketIndex">
											<span class="glyphicon glyphicon-hand-left"></span>
										</s:if>
									</td>
									<td>
										<s:if test="#status.index == ticketIndex">
											<s:url var="skipSeatUrl" action="popupSkipSeat" namespace="/" escapeAmp="false">
												<s:param name="bookingId" value="%{bookingId}" />
												<s:param name="skip" value="true" />
											</s:url> 
											<sj:a button="true" href="%{skipSeatUrl}" value="Skip" targets="checkinBookingDialog" onClickTopics="blockUIOn" onCompleteTopics="blockUIOff">Skip <span class="glyphicon glyphicon-triangle-right"></span></sj:a>
										</s:if>
									</td>
								</tr>
							</s:iterator>
